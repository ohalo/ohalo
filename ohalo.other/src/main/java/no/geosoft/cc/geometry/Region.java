 ...
 * coordinate.
 * <p>
 * Furthermore, the rectangles are banded such that every rectangle with a given
 * upper-left y coordinate (y1) will have the same lower-right y coordinate (y2)
 * and vice versa. If a rectangle has scanlines in a band, it will span the
 * entire vertical distance of the band. This means that some areas that could
 * be merged into a taller rectangle will be represented as several shorter
 * rectangles to account for shorter rectangles to its left or right but within
 * its "vertical scope".
 * <p>
 ...
 * <p>
 * The <em>Region</em> is essentially equivalent to an AWT <em>Area</em> but
 * with different back-end implementation. Becnhmarking proves it more than 100
 * times faster than AWT Area for binary CAG operations,
 * <p>
 * Thanks to:
 * <ul>
 * <li>Bryan Lin @ China Minmetals Corporation - for identifying synchronization
 * errors when run on the MS WindowsXP platform.
 * </ul>
 * 
...
public class Region implements Cloneable {
	 ...
	 * 
	 * @param x
	 *            X values of polygon vertices.
	 * @param y
	 *            Y values of polygon vertices.
	 */
	 ...
	 * 
	 * @param rectangle
	 *            Rectangle to create region from.
	 */
	 ...
	 * 
	 * @param x
	 *            X position of upper left corner of rectangle.
	 * @param y
	 *            Y position of upper left corner of rectangle.
	 * @param width
	 *            Width of rectangle.
	 * @param height
	 *            Height of rectangle.
	 ...
	 * 
	 * @param box
	 *            Box specification of rectangle to create region from.
	 */
	...
	public java.awt.geom.Area createArea() {
		...
		java.awt.Rectangle rectangle = new java.awt.Rectangle(rectangles_[2],
				rectangles_[0], rectangles_[3] - rectangles_[2], rectangles_[1]
						- rectangles_[0]);
		...
		for (int i = 1; i < nRectangles_; i++) {
			...
			rectangle = new java.awt.Rectangle(rectangles_[j + 2],
					rectangles_[j + 0],
					rectangles_[j + 3] - rectangles_[j + 2], rectangles_[j + 1]
							- rectangles_[j + 0]);
	...
	private static void checkMemory(Region region, int nRectangles) {
		...
		if (region == null) {
			if (gRectangles_.length < nEntries) {
				...
				System.arraycopy(gRectangles_, 0, newArray, 0,
						gRectangles_.length);
	...
	public void set(Region region) {
		...
		System.arraycopy(region.rectangles_, 0, rectangles_, 0,
				region.nRectangles_ << 2);
	 ...
	 * 
	 * @param rectangle
	 *            Rectangle to set region according to.
	 */
	 ...
	 *             if object is not of type Region.
	 */
	 ...
	 * 
	 * @param x
	 *            X part of point to check.
	 * @param y
	 *            Y part of point to check.
	 ...
	 * 
	 * @param rectangle
	 *            Rectangle to check if this is inside of.
	...
	private void combine(Region region, int operationType) {
		...
		int yTop = 0;
		int yBottom = extent_.y1 < region.extent_.y1 ? extent_.y1
				: region.extent_.y1;
		...
		do {
			...
			if (rectangles_[r1] < region.rectangles_[r2]) {
				...
				if (top != bottom)
					nonOverlap1(rectangles_, r1, r1BandEnd, top, bottom,
							operationType);
			...
			if (yBottom > yTop)
				overlap(rectangles_, r1, r1BandEnd, region.rectangles_, r2,
						r2BandEnd, yTop, yBottom, operationType);

			// Coalesce again
			if (gNRectangles_ != currentBand)
				previousBand = coalesceBands(previousBand, currentBand);

			// If we're done with a band, skip forward in the region to the next
			// band
	...
	private void nonOverlap1(int rectangles[], int r, int rEnd, int yTop,
			int yBottom, int operationType) {
	...
	private void nonOverlap2(int rectangles[], int r, int rEnd, int yTop,
			int yBottom, int operationType) {
		int i = gNRectangles_ << 2;
	...
	private void overlap(int rectangles1[], int r1, int r1End,
			int rectangles2[], int r2, int r2End, int yTop, int yBottom,
			int operationType) {
		int i = gNRectangles_ << 2;
		...
		if (operationType == OPERATION_UNION) {
			while (r1 != r1End && r2 != r2End) {
				if (rectangles1[r1 + 2] < rectangles2[r2 + 2]) {
					if (gNRectangles_ > 0 && gRectangles_[i - 4] == yTop
	 ...
	 * 
	 * @param r1
	 *            First region to union.
	 * @param r2
	 *            Second region to union.
	 ...
	 * 
	 * @param r1
	 *            First region to intersect.
	 * @param r2
	 *            Second region to intersect.
	 ...
	 * 
	 * @param region
	 *            Region to subtract from this region.
	 */
	 ...
	 * 
	 * @param rectangle
	 *            Rectangle to subtract from this region.
	 */
	 ...
	 * 
	 * @param region
	 *            Region to xor this region with.
	 */
	 ...
	 * 
	 * @param rectangle
	 *            Rectangle to xor this region with.
	 */
	...
	private boolean isCoalesced() {
		...
		while (thisBand != rEnd) {
			...
			if (rectangles_[thisBand + 1] == rectangles_[nextBand + 1]) {
				...
				while (j != rEnd && rectangles_[i] == thisY
						&& rectangles_[j] == nextY) {
				...
				if (rectangles_[i] != thisY
						&& (rectangles_[j] != nextY || j == rEnd))
