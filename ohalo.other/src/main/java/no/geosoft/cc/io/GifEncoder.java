...
public class GifEncoder {
	 ...
	 * 
	 * @param image
	 *            The image to encode. The image must be completely loaded.
	 * @throws AWTException
	 *             If memory is exhausted or image contains more than 256
	 *             colors.
	 */
	public GifEncoder(Image image) throws AWTException {
		...
		PixelGrabber grabber = new PixelGrabber(image, 0, 0, imageWidth_,
				imageHeight_, values, 0, imageWidth_);
	 ...
	 *            File to erite to.
	 */
	public static void writeFile(Image image, File file) throws AWTException,
			IOException {
	...
	/**
	 * Writes the image out to a stream in GIF format. This will be a single
	 * GIF87a image, non-interlaced, with no background color.
	 * 
	 * @param stream
	 *            The stream to which to output.
	 * @throws IOException
	...
	private void writeImageDescriptor(OutputStream stream, short width,
			short height, char separator) throws IOException {
	...
	private class BitFile {
		...
		void writeBits(int bits, int nBits) throws IOException {
			...
			do {
				if ((streamIndex_ == 254 && bitsLeft_ == 0)
						|| streamIndex_ > 254) {
					stream_.write(nBytes);
					stream_.write(buffer_, 0, nBytes);
