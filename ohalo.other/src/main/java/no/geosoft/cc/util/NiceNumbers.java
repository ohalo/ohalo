...
/**
 * Find "nice numbers" within an interval. The interval is given by a min and a
 * max value:
 * 
 * <pre>
 ...
 * </pre>
 * 
 * This class is handy for producing quality annotation on graphic displays, for
 * instance along an axis.
 * 
...
public class NiceNumbers implements Iterator {
	 ...
	 * @param toValue
	 *            To value.
	 * @param nNumbersApprox
	 *            Approximate number of major nice numbers to produce.
	 * @param isBounded
	 *            True if fromValue/toValue should be end points and hence
	 *            reported as nice numbers.
	 */
	public NiceNumbers(double fromValue, double toValue, int nNumbersApprox,
		...
		if (majorStep_ > 7.50)
			majorStep_ = 10.0;
		else if (majorStep_ > 3.50)
			majorStep_ = 5.0;
		else if (majorStep_ > 2.25)
			majorStep_ = 2.5;
		else if (majorStep_ > 1.50)
			majorStep_ = 2.0;
		else
			majorStep_ = 1.0;
		...
		if (majorStep_ == 10.0)
			minorStep_ = 5.0;
		else if (majorStep_ == 5.0)
			minorStep_ = 2.5;
		else if (majorStep_ == 2.5)
			minorStep_ = 0.5;
		else if (majorStep_ == 2.0)
			minorStep_ = 1.0;
		else
			minorStep_ = 0.1;
		...
		if (minorStep_ == 5.0)
			minorMinorStep_ = 1.0;
		else if (minorStep_ == 2.5)
			minorMinorStep_ = 0.5;
		else if (minorStep_ == 1.0)
			minorMinorStep_ = 0.1;
		else if (minorStep_ == 0.5)
			minorMinorStep_ = 0.1;
		else
			minorMinorStep_ = 0.0;
		...
		if (isAscending && firstValue_ > fromValue_)
			firstValue_ -= majorStep_;
		else if (!isAscending && firstValue_ < fromValue_)
			firstValue_ -= majorStep_;

		// Find last nice value after toValue
		lastValue_ = ((int) (toValue_ / majorStep_)) * majorStep_;
		if (isAscending && lastValue_ < toValue_)
			lastValue_ += majorStep_;
		else if (!isAscending && lastValue_ > toValue_)
			lastValue_ += majorStep_;
	...
	public Object next() {
		...
		int rank;
		if (valueNo_ % (int) majorStep_ == 0.0)
			rank = 0;
		else if (valueNo_ % (int) minorStep_ == 0.0)
			rank = 1;
		else
			rank = 2;
		...
		double position = first == last ? 0.0 : (value - first)
				/ (last - first);
	...
	/**
	 * From Iterator. Removing nice numbers are not possible, so this method is
	 * left empty.
	 */
	public void remove() {
		// Not possible
