 ...
 * </pre>
 * 
 * A typical <tt>GObject</tt> will have many <tt>GSegment</tt>s and sub-
 * <tt>GObject</tt>s. Some of these can be created in the constructor while
 * others may need to be created within the draw method as they depend ont
 * external factors such as zoom etc.
 * <p>
 * For efficiency, <b>G</b> does not store world coordinates internally but
 * converts these to device coordinates in the rendering step. It is therfore
 * essential that geometry is provided in the <tt>draw()</tt> method which is
 * called by <b>G</b> on retransformations (zoom/resize etc.).
 * 
...
public class GSegment implements GStyleListener {
	...
	private void updateContext() {
		...
		if (rectangle_ != null)
			isVisible_ = owner_.getScene().getRegion()
					.isIntersecting(rectangle_);
	 ...
	 */
	void updateDamage() {
		if (isVisible_ && rectangle_ != null && !rectangle_.isEmpty()
				&& owner_ != null && owner_.getWindow() != null)
	 ...
	 * 
	 * @param xy
	 *            Polyline geometry [x,y,x,y,...]. null can be specified to
	...
	/**
	 * Set single point world coordinate geometry. Ignore Z coordinate (set to
	 * 0.0).
	 * 
	 * @param x
	 *            X coordinate.
	 ...
	 *            Y coordinate of first end point.
	 * @param z0
	 *            Z coordinate of first end point.
	 * @param x1
	 *            X coordinate of second end point.
	 * @param y1
	 *            Y coordinate of second end point.
	 * @param z1
	 *            Z coordinate of second end point.
	 */
	public void setGeometry(double x0, double y0, double z0, double x1,
			double y1, double z1) {
		setGeometry(new double[] { x0, x1 }, new double[] { y0, y1 },
				new double[] { z0, z1 });
	}

	/**
	 * Set two point (line) world coordinate geometry. Ignore Z coordinate (set
	 * to 0.0).
	 * 
	 * @param x0
	 *            X coordinate of first end point.
	 ...
	 *            Y coordinates.
	 * @param wz
	 *            Z coordinates. May be null if Z is to be ignored.
	 */
	public void setGeometry(double x[], double y[], double z[]) {
	 ...
	 * 
	 * @param style
	 *            Style for this segment (or null if the intent is to unset the
	 *            current style).
	 */
	 ...
	 * 
	 * @param image
	 *            Image to decorate every vertex of this polyline (or null to
	...
	/**
	 * Check if this segment is visible. This is visibility due to geometry
	 * position relative to viewport, <em>not</em> due to GObject visibility
	 * settings.
	 * 
	 ...
	 *         specified rectangle, false otherwise. If this GSegment has no
	 *         geometry, false is returned.
	 */
	 ...
	 *         rectangle, false otherwise. If the GSegment has no geometry,
	 *         false is returned.
	 */
	boolean isIntersectingRectangle(int x0, int y0, int x1, int y1) {
		...
		return (isFilled() && Geometry.isPolygonIntersectingRectangle(x_, y_,
				x0, y0, x1, y1))
				|| (!isFilled() && Geometry.isPolylineIntersectingRectangle(x_,
						y_, x0, y0, x1, y1));
	}
	...
	boolean isIntersectingPoint(int x, int y) {
		...
		return (isFilled() && Geometry.isPointInsidePolygon(x_, y_, x, y))
				|| (!isFilled() && Geometry.isPolylineIntersectingRectangle(x_,
						y_, x - 1, y - 1, x + 1, y + 1));
	}
