...
/**
 * GWindow is the top level graphics node and holder of GScene nodes (node
 * containing world-to-device transformation). The GWindow is linked to the GUI
 * through its canvas object.
 * <p>
 * Typical usage:
 * 
 * <pre>
 ...
 * </pre>
 * 
 * GWindow is also the holder of the current "interaction" object communicating
 * mouse events between the back-end AWT component and the client application.
 * 
...
public class GWindow {
	 ...
	 * <p>
	 * The window contains a JComponent canvas which should be added to a
	 * container widget in the GUI.
	 */
	...
	/**
	 * Return rendering canvas of this window. This is the component that should
	 * be added to the client GUI hierarchy.
	 * 
	...
	/**
	 * Add a scene to this window. A window may have more than one scene. The
	 * first scene added is rendered first (i.e. it appears in the background of
	...
	/**
	 * Return the first scene of this window (or null if no scenes are attached
	 * to this window). This method is a convenience where the client
	 * application knows that are exactly one scene in the window (which in many
	 * practical cases will be the case).
	 * 
	...
	/**
	 * Find scene at the specified location. If there are more than one scene at
	 * the specified location, select the front most.
	 * 
	 * @param x
	 *            X coordinate of location of scene.
	 * @param y
	 *            Y coordinate of location of scene.
	...
	private GScene getScene(int x, int y) {
		for (int i = scenes_.size() - 1; i >= 0; i--) {
			...
			if (Geometry.isPointInsidePolygon(
					new int[] { viewport.getX0(), viewport.getX1(),
							viewport.getX3(), viewport.getX2() },
					new int[] { viewport.getY0(), viewport.getY1(),
							viewport.getY3(), viewport.getY2() }, x, y))
				return scene;
		}
	 ...
	 * 
	 * @param name
	 *            User data of object to search for.
	 ...
	 * 
	 * @param interaction
	 *            Interaction to install and start.
	 */
	 ...
	 * isRegionValid_ flag set to false (and their parents) will be recomputed.
	 */
	void computeRegion() {
		// This is default setting from window point of view
		int visibilityMask = GObject.DATA_VISIBLE | GObject.ANNOTATION_VISIBLE
	 ...
	 * <p>
	 * Normally this method is called automatically when needed (typically on
	 * retransformations). A client application <em>may</em> call this method
	 ...
	 * and consider calling GObject.redraw() on the affected objects instead.
	 */
	public void redraw() {
		// This is default setting from window point of view
		int visibilityMask = GObject.DATA_VISIBLE | GObject.ANNOTATION_VISIBLE
				| GObject.SYMBOLS_VISIBLE | GObject.WIDGETS_VISIBLE;
	...
	/**
	 * Method called when the pointer enters this window. If an interaction is
	 * installed, pass a FOCUS_IN event to it.
	 * 
	 * @param x
	 *            X position of mouse.
	 * @param y
	...
	/**
	 * Method called when the pointer exits this window. If an interaction is
	 * installed, pass a FOCUS_OUT event to it.
	 * 
	 * @param x
	 *            X position of mouse.
	 * @param y
	...
	/**
	 * Method called when a mouse pressed event occurs in this window. If an
	 * interaction is installed, pass a BUTTON*_DOWN event to it.
	 * 
	 * @param buttonEvent
	 *            Button event trigging this method.
	 * @param x
	 *            X position of mouse.
	 * @param y
	 *            Y position of mouse.
	...
	/**
	 * Method called when a mouse release event occurs in this window. If an
	 * interaction is installed, pass a BUTTON*_UP event to it.
	 * 
	 * @param buttonEvent
	 *            Button event trigging this method.
	 * @param x
	 ...
	 * this window. If an interaction is installed, pass a BUTTON*_DRAG event to
	 * it.
	 * 
	 * @param buttonEvent
	 *            Button event trigging this method.
	...
	/**
	 * Method called when the mouse is moved inside this window. If an
	 * interaction is installed, pass a MOTION event to it.
	 * 
	 * @param x
	 *            X position of mouse.
	 * @param y
	...
	/**
	 * Called when the window is resized. Reset the dimension variables and
	 * resize scenes accordingly.
	 */
	void resize() {
