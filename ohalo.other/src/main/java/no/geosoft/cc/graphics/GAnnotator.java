...
/**
 * Layout manager for positionals. Positionals are object attached to graphic
 * elements by means of rendering hints.
 * 
...
class GAnnotator {
	...
	/**
	 * Create annotator for the specified scene. The annotator use the scene
	 * viewport as its canvas, and position all positional elements of the scene
	 * inside this region.
	 * 
	 * @param scene
	 *            Scene of this annotator.
	 */
	...
	/**
	 * Reset this annotator. The annotator must be reset when the scene viewport
	 * is changed.
	 */
	void reset() {
	...
	/**
	 * Compute the x and y position of the specified positional elements and set
	 * isVisible flag in case they are outside the screen. After this
	 * computation is done on all nodes, the positionals can be rendered
	 * directly by the GCanvas object.
	 * 
	 * @param positionals
	 *            Elements to position (of GPositional).
	 */
	...
	void computePositions(Collection positionals) {
		...
		for (Iterator i = positionals.iterator(); i.hasNext();) {
			...
			if (segment.isVisible()) {
				...
				// Find the preferred annotation according to line position
				// hints
				...
				if ((positionHint & GPosition.DYNAMIC) != 0
					...
					distance_ = 0.0;
					if (!scene_.getRegion().isInside(
							positional.getRectangle().x,
							positional.getRectangle().y))
						pointNo = moveInsideWindow(positional, x, y, pointNo);
				} else {
					positional.setVisible(scene_.getRegion().isInside(
							positional.getRectangle().x,
							positional.getRectangle().y));
				}
				...
				if (positional.isVisible()) {
					...
					if ((positionHint & GPosition.STATIC) != 0
						...
						if (!positional.isAllowingOverlaps()
							...
							findNonOverlappingPosition(positional, x, y,
									pointNo);
					}
				}
			}
			...
			if (!oldRectangle.equals(positional.getRectangle())) {
				...
				if (positional.isVisible())
					scene_.getWindow().updateDamageArea(
							positional.getRectangle());
	...
	/**
	 * Find the preferred position of a positional element. This is the position
	 * as dictaed by its rendering hint and before any adjustments are done.
	 * 
	 * @param positional
	 *            Positional element to position.
	 * @param x
	 *            X values of polyline to position according to.
	 * @param y
	 *            Y values of polyline to position according to.
	 * @param x0
	 *            X coordinate of center of gravity of polyline (used with
	 *            rendering hint MIDDLE).
	 * @param y0
	 *            Y coordinate of center of gravity of polyline (used with
	 *            rendering hint MIDDLE).
	 * @param pointNo
	 *            Index number of this positional on this polyline (used when no
	 *            line rendering hints are given).
	 * @return Index in x,y of point to attach this positional to.
	 */
	private int findPreferredPosition(GPositional positional, int x[], int y[],
			int x0, int y0, int pointNo) {
		int positionHint = positional.getPositionHint();
		int nPoints = x.length;

		if (positional.isLinePositional()) {

			// Find the point on the segment that the positional should attach
			// to
	...
	/**
	 * Adjsut the position of the specified positional according to point
	 * position hint.
	 * 
	 * @param positional
	 *            Positionla to adjust.
	 */
	 ...
	 * 
	 * @param positional
	 *            Positional to adjust.
	 * @param x
	 *            X values of polyline to position along.
	 * @param y
	 *            Y values of polyline to position along.
	 * @param pointNo
	 *            Index in polyline where positional is initially attached.
	 */
	private void findNonOverlappingPosition(GPositional positional, int x[],
			int y[], int pointNo) {
		...
		if (distance_ == 0.0) {
			...
			x0 = (int) Math.round(x[pointNo] + fraction
					* (x[pointNo + 1] - x[pointNo]));
			y0 = (int) Math.round(y[pointNo] + fraction
					* (y[pointNo + 1] - y[pointNo]));
		}
		...
		while (!isDone) {
			...
			if (!isDoneLeft) {
				isDoneLeft = !Geometry
						.findPolygonPosition(x, y, left, position);
				if (!isDoneLeft) {
			...
			// Check to the right
			if (!isDoneRight) {
				isDoneRight = !Geometry.findPolygonPosition(x, y, right,
						position);
				if (!isDoneRight) {
	...
	/**
	 * Given a polyline (x[], y[]) and a position (pointNo) on this polyline for
	 * the positional. This function is called because we know that pointNo is
	 * outside the visible. We find the closest point (along line) that is
	 * inside. If none is found the positional is set invisible and no further
	 * positioning is required. If a point is found the positional rectangle is
	 * set to the viewport intersection point, the polyline point is returned
	 * and the distance_ variable is set as the distance from the positional to
	 * the returned point.
	 * 
	 * @param positional
	 *            Positional to move inside window.
	 * @param x
	 *            X values of polyline.
	 * @param y
	 *            Y values of polyline.
	 * @param pointNo
	 *            Index of polyline where positional is initially attached.
	 * @return New index to attach positional to.
	 */
	private int moveInsideWindow(GPositional positional, int x[], int y[],
			int pointNo) {
		...
		for (iPoint = pointNo - 1; iPoint >= 0; iPoint--) {
			intersection = findViewportIntersection(x[iPoint + 1],
					y[iPoint + 1], x[iPoint], y[iPoint]);
	...
	/**
	 * Find the intersection point of the specified line segment and the current
	 * viewport.
	 * 
	 * @param x0
	 ...
	 * @param x1
	 *            X coordinate of second line segment endpoint.
	 * @param y1
	 *            Y coordinate of second line segment endpoint.
	 * @return Coordinate of intersection (or null if the specified line segment
	 *         doesn't intersect the viewport).
	 */
	private int[] findViewportIntersection(int x0, int y0, int x1, int y1) {
		...
		// Top viewport edge
		type = Geometry.findLineSegmentIntersection(x0, y0, x1, y1, vx0_, vy0_,
				vx1_, vy1_, intersection);
		if (type == 1l) {
		...
		// Right viewport edge
		if (n < 2) {
			type = Geometry.findLineSegmentIntersection(x0, y0, x1, y1, vx1_,
					vy1_, vx3_, vy3_, intersection);
			if (type == 1) {
				x[n] = (int) Math.round(intersection[0]);
				y[n] = (int) Math.round(intersection[1]);
				length[n] = Geometry.length(x0, y0, x[n], y[n]);
		...
		if (n < 2) {
			type = Geometry.findLineSegmentIntersection(x0, y0, x1, y1, vx2_,
					vy2_, vx3_, vy3_, intersection);
			if (type == 1) {
				x[n] = (int) Math.round(intersection[0]);
				y[n] = (int) Math.round(intersection[1]);
				length[n] = Geometry.length(x0, y0, x[n], y[n]);
	...
	/**
	 * This method is used to position a positional on every vertex along a
	 * polyline. The positioanl rectangle x and y values are computed as delta
	 * measure, so that the positionals can later be positioned by adding this
	 * delta to each vertex.
	 * 
	 * @param positional
	 *            Positional to compute position of.
	 */
