...
public class GObject implements GStyleListener {
	 ...
	 * 
	 * @param name
	 *            New name of this graphic object.
	 */
	 ...
	 * 
	 * @return Window of this graphic object (or null if this object is not
	...
	public GSegment getSegment() {
		return segments_ != null && segments_.size() > 0 ? (GSegment) segments_
				.get(0) : null;
	}
	 ...
	 * 
	 * @param isValid
	 *            True if region is valid, false if it is invalid.
	 */
	 ...
	 * 
	 * @param child
	 *            Child object to be repositioned.
	 * @param position
	...
	public GObject find(int x0, int y0, int x1, int y1) {
		...
		return objects.size() == 0 ? null : (GObject) objects.get(objects
				.size() - 1);
	}
	 ...
	 * 
	 * @param segment
	 *            Segment to remove. If the specified segment is not a child of
	 *            this GObject, this call has no effect.
	 */
	 ...
	 * 
	 * @param segmentNo
	 *            Segment to remove. If the specified segment does not exist,
	 *            this casll has no effect.
	 */
	 ...
	 *            Starting segment
	 * @param to
	 *            Ending segment (-1 indicates all).
	 */
	 ...
	 * 
	 * @param style
	 *            Style of this object (or null if the intent is to unset the
	 *            current style).
	 */
	...
	public void setVisibility(int visibilityMask) {
		...
		if ((oldVisibilityMask & SYMBOLS_VISIBLE) != (visibilityMask_ & SYMBOLS_VISIBLE))
			changeSymbolVisibility();

		// Widget visibility has changed
		if ((oldVisibilityMask & WIDGETS_VISIBLE) != (visibilityMask_ & WIDGETS_VISIBLE))
			changeWidgetVisibility();

		// Annotation visibility has changed
		if ((oldVisibilityMask & ANNOTATION_VISIBLE) != (visibilityMask_ & ANNOTATION_VISIBLE)) {
		...
		if ((oldVisibilityMask & DATA_VISIBLE) != (visibilityMask_ & DATA_VISIBLE)) {
	...
	/**
	 * Compute position of all GComponents in the subtree rooted at this
	 * GObject.
	 */
	...
	/**
	 * Compute position of all GImages in the subtree rooted at this GObject.
	 */
	...
	/**
	 * Convenience method for refreshing the window canvas. Equivalent to
	 * <code>getWindow().refresh();</code>.
	 */
	public void refresh() {
		GWindow window = getWindow();
		if (window != null)
